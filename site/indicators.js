...
(async () => {
  ...
  function render() {
    ...
    q("#series-grid").innerHTML = visible.map(item => `<article class="series-card"><span class="goal">Goal ${item.goal}: ${goalMap.get(String(item.goal))?.label || "Unknown goal"}</span><h3>${escapeHTML(item.description)}</h3><p>${escapeHTML(item.rationale)} · Expected every ${item.cadence} year${item.cadence === 1 ? "" : "s"}.</p><div class="coverage-bar"><i style="width:${item.coverage}%"></i></div><span class="coverage-label">${item.coverage.toFixed(1)}% recent coverage · ${item.applicability}</span><span class="breakdown-label">Breakdowns · sex ${item.disaggregation_coverage.sex.toFixed(0)}% · age ${item.disaggregation_coverage.age.toFixed(0)}% · location ${item.disaggregation_coverage.location.toFixed(0)}%</span><a href="https://unstats.un.org/sdgs/metadata/?Goal=${item.goal}" rel="noreferrer">Official goal metadata ↗</a></article>`).join("");
