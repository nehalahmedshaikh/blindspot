...
function showTooltip(event, country) {
  ...
  tip.innerHTML = `<strong>${country.name}</strong><br>${labelForMetric()}: ${countryValue(country)?.toFixed(1) ?? "n/a"}`;
...
function selectCountry(alpha3) {
  ...
  const gaps = state.data.country_series.filter(item => item.country_alpha3 === alpha3 && item.measurement_priority_v1 != null).sort((a,b) => b.measurement_priority_v1 - a.measurement_priority_v1).slice(0,3);
  q("#country-profile").innerHTML = `<div class="profile-head"><div><small>${country.region} · ${country.income_group}</small><h3>${country.name}</h3></div><div class="profile-stat"><strong>${country.completeness.toFixed(0)}%</strong><span>complete</span></div><div class="profile-stat"><strong>${country.staleness.toFixed(0)}</strong><span>staleness</span></div><div class="profile-stat"><strong>${country.priority.toFixed(0)}</strong><span>priority</span></div></div><div class="profile-gaps">${gaps.map(gap => `<article><b>Goal ${gap.goal} · ${gap.series_code}</b><span>${seriesMap.get(gap.series_code)?.description || gap.series_code}</span><span>Latest: ${gap.latest_year || "no observation since 2015"} · Score ${gap.measurement_priority_v1.toFixed(1)}</span></article>`).join("")}</div>`;
}
...
function renderRankings() {
  ...
  q("#ranking-body").innerHTML = ranked.map((item,index) => `<tr><td>${String(index+1).padStart(2,"0")}</td><td>${item.country_name}</td><td>${item.goal}</td><td>${seriesMap.get(item.series_code)?.description || item.series_code}</td><td>${item.latest_year || "—"}</td><td>${item.adjusted.toFixed(1)}</td></tr>`).join("");
}

function renderSeries(filter="") {
  const needle = filter.toLowerCase().trim();
  const rows = state.data.series.filter(item => `${item.goal} ${item.code} ${item.description}`.toLowerCase().includes(needle));
  q("#series-grid").innerHTML = rows.map(item => `<article class="series-card"><span class="goal">Goal ${item.goal} · ${item.code}</span><h3>${item.description}</h3><div class="coverage-bar"><i style="width:${item.coverage}%"></i></div><span class="coverage-label">${item.coverage.toFixed(1)}% recent coverage · ${item.applicability}</span></article>`).join("");
}

function populateControls() {
  for (let goal=1; goal<=17; goal++) q("#goal-select").insertAdjacentHTML("beforeend", `<option value="${goal}">Goal ${goal}</option>`);
...
async function init() {
  try {
    ...
    const selected=state.data.model.selected.replaceAll("_"," "); q("#model-summary").textContent=`Temporal backtesting selected ${selected}. Experimental predictions are descriptive, not causal.`;
